...
with open(fname, 'r') as f:
    ...
    # in the list 'data'
    #AG: users_with_geodata is a dictionary with 1 key  such that   users_with_geodata["data"] is a list
    users_with_geodata = {
        "data": []
    }
    all_users = []
    ...
    for line in f:
        ...
        if tweet['user']['id']:
            ...
            if user_id not in all_users: #unique users only
                all_users.append(user_id) #filter for people that tweet a lot (bots and so on)

                #Give users some data to find them by. User_id listed separately
                # to make iterating this data later easier
            #AG: define dictionary with two keys, users_data['features'] will be a dictionary itself
                user_data = {
                    "user_id" : tweet['user']['id'],
                    "features" : {
                ...
                #after extracting the location we can append the new element into the list users_with_geodata["data"]
                ...
                if user_data["features"]["primary_geo"]:
                    #append the dictionary user_data as a new element in the list
                    users_with_geodata['data'].append(user_data) #the append method applies after indexing !
                    geo_tweets += 1
